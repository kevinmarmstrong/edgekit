...
- Create and verify external admin and docs demos before retiring `site/src/adminDemo.ts`, docs Q&A runtime, the site assistant demo surface, cascade lab, ops demo, and AG-UI mock.
- Move `examples/cloudflare-sidecar` to an external demo repo or `lab/`.
- Finish with `/site/src/` containing marketing-only code and no demo runtime.
