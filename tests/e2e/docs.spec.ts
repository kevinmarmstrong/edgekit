...
test('homepage links into the full documentation site', async ({ page }) => {
  ...
  await expect(page.locator('#doc-card-grid a.doc-card')).toHaveCount(32)
  ...
  await expect(page.locator('a.doc-card[href="/edgekit/docs/public-site-qa/"]')).toBeVisible()
...
test('docs pages expose core documentation sections and navigation', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Deprecated routing experiments' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'Worker handoffs' })).toBeVisible()
