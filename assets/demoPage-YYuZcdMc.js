const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/main-BgYMHMQi.js","assets/siteAssistant-swUzf04r.js","assets/styles-Cr5wP9I3.js","assets/styles-BCvRgNPm.css"])))=>i.map(i=>d[i]);
import{t as e}from"./styles-Cr5wP9I3.js";var t=`/edgekit/`.replace(/\/$/,``),n=document.querySelector(`#demo-root`),r=document.body.dataset.demoPage??`ecommerce`,i={ecommerce:{title:`Ecommerce retrofit demo`,label:`Live ecommerce`,summary:`Product search, selectable CTAs, and guarded cart mutation inside a storefront workflow.`},operations:{title:`Field ops ERP demo`,label:`Field ops ERP`,summary:`Work-order triage, inventory reservation, and technician dispatch inside an operational ERP surface.`},docs:{title:`Docs Q&A demo`,label:`Live docs Q&A`,summary:`Project knowledge exposed as a registered search tool, with a fallback docs search path.`},"ag-ui":{title:`AG-UI event stream demo`,label:`AG-UI compatible`,summary:`Remote-style event rendering for generated forms, charts, tables, and cards.`},admin:{title:`SaaS admin workflow demo`,label:`Live SaaS admin`,summary:`Account search, plan changes, and suspension tools with explicit approval gates.`},"mission-control":{title:`Mission control demo`,label:`Telemetry primitive`,summary:`Local run, tool, approval, and model-availability telemetry from a deployed Edgekit sidecar.`}};if(n){let e=i[r];document.title=`${e.title} · edgekit demos`,n.innerHTML=`
    <header class="site-header">
      <a class="brand" href="${m(`/`)}" aria-label="edgekit home">
        <span class="brand-mark">ek</span>
        <span>edgekit</span>
      </a>
      <nav aria-label="Demo navigation">
        <a href="${m(`/docs/`)}">Docs</a>
        <a href="${m(`/#demos`)}">Demos</a>
  ...
  `}e(()=>import(`./main-BgYMHMQi.js`).then(e=>e.t),__vite__mapDeps([0,1,2,3]));function a(){return Object.keys(i).map(e=>{let t=i[e];return`<a class="${e===r?`current`:``}" href="${m(`/demos/${e}/`)}">${t.label}</a>`}).join(``)}function o(e){return e===`docs`?s():e===`ag-ui`?u():e===`admin`?d():e===`operations`?l():e===`mission-control`?f():c()}function s(){return`
    <section class="split-section" id="qa">
      <div>
        ...
        ${p([`Replace the in-memory docs index with your own search or retrieval API.`,`Keep the docs search tool read-only and cacheable when possible.`,`Use outcome-quality tests to reject generic docs-search snippets.`])}
  ...
  `}function c(){return`
    <section class="ecommerce-demo" id="ecommerce">
      <div class="section-heading">
        ...
        ${p([`Replace the sample catalog with your app-owned product search API.`,`Keep checkout and cart mutations behind explicit approval.`,`Use telemetry to track searches, action-card clicks, approvals, and failures.`])}
      </div>
  ...
  `}function u(){return`
    <section class="split-section" id="agui">
      <div>
        ...
        ${p([`Serve AG-UI events from a backend route that owns provider secrets and rate limits.`,`Map provider events into EdgeView or A2UI-compatible payloads.`,`Keep submitted forms wired to app-owned tools rather than provider-owned side effects.`])}
      </div>
      <edge-chat
        id="agui-chat"
  ...
  `}function d(){return`
    <section class="admin-demo" id="admin">
      <div class="section-heading">
        ...
        ${p([`Bind tool manifests to the signed-in user and tenant.`,`Keep account mutations behind approval, audit, and backend authorization.`,`Never place tokens or secret claims in state summaries or prompts.`])}
      </div>
  ...
  `}function f(){return`
    <section class="mission-section" id="mission-control">
      <div class="section-heading">
        ...
        ${p([`Forward telemetry to your observability stack with tenant and session identifiers.`,`Track tool calls, approvals, rejections, model availability, and errors.`,`Keep mission-control visibility separate from user-facing agent behavior.`])}
      </div>
  ...
  `}function m(e){return e===`/`?`${t}/`:e.startsWith(`/#`)?`${t}/${e.slice(1)}`:`${t}${e}`}