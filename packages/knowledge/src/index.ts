...
function isWeaklySupported(input: string, results: EdgeKnowledgeResult[]): boolean {
  ...
  const terms = tokenize(input).filter(term => !groundingStopWords.has(term))
  if (terms.length === 0) return true
  const candidates = results.slice(0, 3)
  const text = candidates.map(result => `${result.title || ''} ${result.excerpt || ''}`).join(' ').toLowerCase()
  const overlap = new Set(terms.filter(term => text.includes(term))).size
  const requiredOverlap = terms.length <= 2 ? 1 : 2
  if (overlap >= requiredOverlap && overlap / terms.length >= 0.3) return true
  const bestScore = Math.max(...candidates.map(result => typeof result.score === 'number' ? result.score : 0))
